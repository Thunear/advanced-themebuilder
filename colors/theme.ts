...
export const generateColorScale = (
  ...
  const colors = R.mapObjIndexed((colorData) => {
    const { saturation, interpolation, lightness } =
      colorMetaData[colorData.name];
    ...
    return {
      ...
      hex: chroma(interpolColor)
        .luminance(
          getLuminanceFromLightness(lightness[colorScheme]),
          interpolation || "rgb"
        )
        .hex() as CssColor,
    };
...
const generateBaseColors = (
  ...
  const baseHoverInterpolColor = chroma(
    L,
    C * colorMetaData["base-hover"].saturation[colorScheme],
    H,
    "oklch"
  ).hex();
  const baseActiveInterpolColor = chroma(
    L,
    C * colorMetaData["base-active"].saturation[colorScheme],
    H,
    "oklch"
  ).hex();
    ...
    default:
      colorScheme === "light"
        ...
        : (chroma(baseDefaultInterpolColor)
            .luminance(
              getLuminanceFromLightness(colorLightness),
              colorMetaData["base-default"].interpolation
            )
            .hex() as CssColor),
