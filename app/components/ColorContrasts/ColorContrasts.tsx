...
export const ColorContrasts = () => {
  ...
  const [reducedLight, setReducedLight] = useState({
    horizontalRange: initialTheme[internalColorScheme].filter((color) =>
      staticHorNames.includes(color.name)
    ),
    verticalRange: initialTheme[internalColorScheme].filter((color) =>
  ...
  useEffect(() => {
    ...
    setReducedLight({
      horizontalRange: newTheme[internalColorScheme].filter((color) =>
        staticHorNames.includes(color.name)
      ),
      verticalRange: newTheme[internalColorScheme].filter((color) =>
        staticVerNames.includes(color.name)
      ),
    });
  }, [selectedColor, colors, internalColorScheme]);
  ...
  }, [selectedBaseColor, colors, internalColorScheme]);
